...
- Identifying key concepts and entities in handover transcripts. In this study, this task was operationalised as a checklist of items recommended for nursing clinical handover at our institution as part of quality-improvement initiatives. The checklist items are summarised in Table @tbl-checklist-items.
...
We used the publicly available NICTA Synthetic Nursing Handover Dataset, which contains realistic recordings of clinical handovers delivered by a registered nurse based on patient profiles with cardiovascular, neurological, renal, and respiratory conditions.^5^ In this dataset, handover monologues were generated from comprehensive patient profiles that included information such as the patient’s name, age, admission history, inpatient duration, and the familiarity between the nurses giving and receiving the handover. The nurse was instructed to simulate a bedside shift-to-shift handover within a medical ward setting.^5^
...
Annotated handover records were first partitioned deterministically into optimisation (75%) and evaluation (25%) subsets using a fixed random split. Model orchestration used DSPy (`dspy`) for predictor construction, configuration, and optimization.

We defined initial prompts as DSPy signature that described the intent of each task. The DSPy signature specified the input and output schema for the tasks, as well as the constraints and evaluation metrics to guide optimization. The span and checklist tasks used task-specific signatures and DSPy optimization workflows. In this report, SBAR span extraction compares:
...
Performance was summarized with standard classification metrics for all tasks:
