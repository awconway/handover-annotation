...
import srsly
from data.dataset import UNCERTAINTY_ALLOWED_LABELS
...
def _valid_gold_spans_from_text_and_spans(
    ...
    """Return valid uncertainty gold spans with explicit char boundaries."""
    spans = raw_spans or []
    text_len = len(text)
    ...
    for span in spans:
        ...
        if label not in UNCERTAINTY_ALLOWED_LABELS:
            continue
...
def _span_items_from_gold_spans(
    ...
    """Return de-duplicated uncertainty label/quote items from raw spans."""
    ...
    for span in valid_spans:
        ...
        if key in seen:
            continue
        seen.add(key)
        items.append(UncertaintyItem(label=span["label"], quote=quote))

    return items


def span_items_from_record(record: dict[str, Any]) -> list[UncertaintyItem]:
    """Return valid uncertainty span items from a raw Prodigy-style record."""
...
def exact_match_metrics(
    gold_items: list[UncertaintyItem], pred_items: list[UncertaintyItem]
) -> dict[str, float]:
...
def _to_langextract_example(lx: Any, record: UncertaintyRecord) -> Any:
...
def _extract_items_from_prediction(prediction: Any) -> list[UncertaintyItem]:
    docs: list[Any]
    ...
    for doc in docs:
        ...
        for extraction in extractions:
            ...
            if not isinstance(label, str) or label not in UNCERTAINTY_ALLOWED_LABELS:
                continue
            ...
            if key in seen:
                continue
            seen.add(key)
            items.append(UncertaintyItem(label=label, quote=quote))
...
def _records_from_dspy_examples(examples: list[Any]) -> list[UncertaintyRecord]:
    records: list[UncertaintyRecord] = []
    for ex in examples:
...
def _items_to_pred_items(items: list[UncertaintyItem]) -> list[dict[str, str]]:
...
def _eval_row_for_record(
    *,
    ...
    pred_items: list[UncertaintyItem],
    span_metrics: dict[str, Any],
) -> dict[str, Any]:
    """Build an eval JSONL row with the same schema as run_eval_uncertain_span.py."""
    return {
        "example": {
            "text": record.text,
...
def iou_span_metrics(
    *, text: str, gold_spans: list[dict[str, Any]], pred_items: list[UncertaintyItem]
) -> dict[str, Any]:
...
def run_langextract_uncertainty_experiment(
    *,
    data_file: str,
    output_file: str,
    model_id: str,
    ...
    if use_dataset_test_split:
        ...
        if not training_pool:
            raise ValueError(
                "No uncertainty training records found in dataset train split."
            )
        if not held_out_pool:
            raise ValueError("No uncertainty test records found in dataset test split.")
        ...
        pool = load_uncertainty_records(data_file, annotator_id=annotator_id)
        if len(pool) < 2:
            raise ValueError("Not enough uncertainty records found to run an experiment.")
    ...
    for record in held_out_records:
        ...
        if raw_prediction is None:
            ...
            print(
                "LangExtract uncertainty giving empty prediction after "
